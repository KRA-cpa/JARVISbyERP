...
import { useRoleTypeMutations, useCompanies } from '../hooks/useAPI';
import { API } from '../api/googleSheet';

/**
 * AdminRoleTypeCreatePage Component
 *
 * Universal Entity Architecture - Create new role types
 * - Full-page form for role type creation
 * - Company assignment (global or company-specific)
 * - Integration with custom field creation workflow
 * - Validation and error handling
 */
const AdminRoleTypeCreatePage = () => {
  ...
  const { createRoleType } = useRoleTypeMutations();
  ...
  const validateForm = () => {
    ...
    if (!formData.name.trim()) {
      errors.name = 'Role type name is required';
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const result = await createRoleType(roleTypeData);
      success('Role type created successfully');

      // Clear draft
      localStorage.removeItem('roleTypeDraft');

      // Navigate to list or offer to create custom fields
      if (window.confirm('Role type created! Would you like to create custom fields for this role type?')) {
        navigate(`/admin/custom-fields/create?entityCategory=role&entityType=${result.id}`);
      } else {
        navigate('/admin?tab=role-types');
      }
    } catch (err) {
      showError(err.message || 'Failed to create role type');
    } finally {
      setLoading(false);
    }
  };
  ...
  const saveDraft = () => {
    localStorage.setItem('roleTypeDraft', JSON.stringify(formData));
    setDraftSaved(true);
  ...
  useEffect(() => {
    const savedDraft = localStorage.getItem('roleTypeDraft');
    if (savedDraft) {
      try {
  ...
  useEffect(() => {
    if (process.env.NODE_ENV === 'development') {
      ...
      const requiredIcons = [
        ...
        'Shield', 'Info', 'Warning', 'Error'
      ];

      // Hook validation using already-available hook results
      validateComponentReferences('AdminRoleTypeCreatePage', {
        icons: requiredIcons,
        hooks: [
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="py-6">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <ol className="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li>
                  <div className="flex items-center">
                    ...
                    <button
                      onClick={() => navigate('/admin?tab=role-types')}
            ...
            {/* Page Header */}
            <div className="mb-8">
              <h1 className="text-3xl font-bold text-gray-900">Create New Role Type</h1>
              <p className="mt-2 text-sm text-gray-600">
                Define a new category for roles with configurable custom fields and permission structures.
              </p>
              {draftSaved && (
                      ...
                      <div className="space-y-6">
                        ...
                        <div>
                          ...
                          <input
                            ...
                            placeholder="e.g., Approver, Administrator, Viewer"
                          />
                          <p className="mt-1 text-xs text-gray-500">
                            A descriptive name for this role category
                          </p>
                          {formErrors.name && (
                        ...
                        <div>
                          ...
                          <textarea
                            ...
                            placeholder="Describe the permissions and responsibilities of this role type..."
                          />
                          <p className="mt-1 text-xs text-gray-500">
                            Optional description to help users understand the role's purpose and permissions
                          </p>
                        </div>
                        ...
                        <div>
                          ...
                          <p className="mt-1 text-xs text-gray-500">
                            Choose whether this role type is available globally or specific to one company
                          </p>
                        </div>
              ...
              <div className="space-y-6">
                  ...
                  <div className="space-y-3 text-sm">
                    <div>
                      ...
                      <p className="text-blue-700">
                        Role types use the same custom field system as tickets and user profiles
                      </p>
                    </div>
                    <div>
                      <h4 className="font-medium text-blue-900">Common Role Types</h4>
                      <p className="text-blue-700">
                        Approver, Administrator, Viewer, Manager, Supervisor, Auditor
                      </p>
                    </div>
                    <div>
                      <h4 className="font-medium text-blue-900">Custom Fields</h4>
                      <p className="text-blue-700">
                        After creation, add fields like permission level, access scope, approval limits
                      </p>
                    </div>
                    <div>
                ...
                <div className="bg-green-50 border border-green-200 rounded-lg p-6">
                  ...
                  <div className="space-y-2 text-sm text-green-700">
                    <p>1. Create the role type</p>
                    <p>2. Add custom fields (permission level, access scope, etc.)</p>
                    <p>3. Define workflow step approvers</p>
                    <p>4. Assign roles to users with date-based validity</p>
                  </div>
                </div>
              </div>
            </div>
...
export default AdminRoleTypeCreatePage;